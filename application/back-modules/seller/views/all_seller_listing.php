<div class="box-body table-responsive no-padding">
    <table class="table table-hover">
        ...
        <tbody>
            <?php $pag=$sn-1; $sn = $sn;
            if(!empty($seller)){
                foreach($seller as $get){?>
            <tr>
                <td><?php echo $sn;?></td>
                <?php if(!empty($get->profile_img)){ 
                    $url = base_url()."../upload/profile/seller/thumb/".$get->profileImage;
                }else{ 
                ...
                <td>
                    <?php if($get->status == 1) : ?>
                    <a href="<?php echo base_url()."seller/activeSeller/" . $get->id; ?>"  class="label label-danger" title="Disapproved"><i class="fa fa-close"></i></a>
                    <?php else : ?>
                    <a href="<?php echo base_url()."seller/activeSeller/" . $get->id; ?>" class="label label-success"  title="Approved"><i class="fa fa-check"></i></a>
                    <?php endif; ?>
                </td>
            </tr>
            <?php $sn++; } } else{ ?>
